...
** email, social media account on website
   
** whois enumeration
   #+begin_src bash
whoos <domain>
   #+end_src
   + TODO wireshark whois traffic
   + whois.arin.net
   + grep name, etc.
     
** google search pattern(site:<site>, filetype:<php>)
   + ref: [[https://www.exploit-db.com/google-hacking-database][Doodle Hacking Database]] 
   + TODO list possible search pl
...
*** the x86 register and their nomenclature
    ...
    + the table of x86 register structure
      ...
      + E[ABCD]X, ESI, EDI: often used as general purpose registers to store temporary data, [[https://wiki.skullsecurity.org/Registers][more info on register (Skull Security, 2012)]] but the primary purpose are as below (quoted OSCP p348):
        ...
        - *ESP* (stack pointer): most recently referenced location on the stack(pointer to the top of the stack)
        - *EBP* (base pointer): points to the top of the stack when a function is called, provide easy reference for the function to its own stack frame (via offsets). i.e. the (offset 0 pointer)
...
r2 = requests.post(urls, <data>, <json>, [...args])
    #+end_src
S2 python#scripting
...
*** basic commands
    ...
    + n(ext)                             # next line
    + s(tep)                             # step into
    + l(ist) [lineNumber]                # list src code
    + b(reakpoint) <functionName>
    + c(ontinue)
    + p(rint) <varName> [=<nuwValue>]    # print/alter an var
    + print *<pointerName>               # print len(*p)
    + print *<pName>[@<arrLength>]       # print *p (e.g. array elements)
    + r(un)                              # (re)run program
    + info locals                        # list local vars
    + make <fileName>                    # makefile
    + disable                            # disable all breakpoints
      
    + layout next                        # show src in GUI
    + layout reg                         # show reg in GUI
    + refresh                            # refresh the GUI
    + backtrace full                     # backtrace, use after crach
