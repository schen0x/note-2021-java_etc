...
**** steps: The Ticket-Granting Service (TGS) Exchange (RFC 4120::3.3)
  ...
  #+end_quote
    - 1. client obtain a *TGT* . Client build  the *KRB_TGS_REQ* , which includes 
  #+begin_quote
  ...
  #+end_quote
    - 1. user wishes to access resources of the domain (e.g. something with a registered *SPN*): client constructs a *TGS_REQ* (Ticket Granting Service Request) packet "that consists of the current user and a timestamp (encrypted using the session key), the SPN of the resource, and the encrypted TGT." (and the session key)
