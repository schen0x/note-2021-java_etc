...
*** Kerboros Authentication
    ...
    + steps::client_auth
      ...
      [[./img/b3cp21_KerberosTGT.png]]
        + the *AS_REP* also contains a session key since Kerberos is stateless. the session key is encrypted using the *hmac*, which maybe decrypted and reused by the client.
