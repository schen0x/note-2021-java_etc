...
*** Pass the Ticket (export a TGS, re-injected to another machine and use)
    ...
    + under the hood:
      ...
      - in other words, the application server trust the TGS blindly. Which means, knowing the service NTLM hash, a TGS with any level of access (any group membership) could be forged. This custom-created ticket is known as a *silver ticket* . "If the specific SPN is used on multiple servers, the silver can be leveraged against them all." (PWK, p663)

    + src:
    #+begin_src
      ## forge a ticket with Mimikatz kerberos::golden command.
    #+end_src 
