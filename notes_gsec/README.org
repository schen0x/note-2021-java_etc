...
*** netcat //has several versions: nmap-ncat, nc
    + //the original version, ~nc~:
    + nc -[u]vnlp 1337 //[u]dp, verbose, nodns, listening, port
    + nc -[u]vn 127.0.0.1 1337
    + nc -znv[u] [ip] 1-100; //(stealth)scan for listening daemons without sending any data to them. 
    + //the nmap maintained version, ~ncat~: ipv6 support, ssl, etc.
...
**** TODO stealth scan (gsec stealth port scanning methods 1985)
    //TODO how nc stealth scan work. (wireshark) (it is definitely not stealth because it break nc listening process.)
    + Inverse Mapping
