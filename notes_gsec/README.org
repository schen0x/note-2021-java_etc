...
*** basic syntax
**** double quotes, single quotes, dollar sign
...
**** command substitution* and *subshell*:
    1. dollar sign and single parenthesis: ~$(command)~ means:
       + execute the command in the parens in a subshell and return its stdout. Example:
         #+begin_src
         user=$(whoami)
         echo $user 
         #+end_src
...
**** bash *EXPANSIONs*: 7 kinds of expansions. including dollar sign, with or without single/double parenthesis
...
***** 3. parameter expansion
      + # The `$' character introduces parameter expansion, command substitution, or arithmetic expansion.
          ...
          a=; echo ${a:?errorMsg}   # a is null | unset, output errorMsg to stderr
          a=2; echo ${a:+$b}        # nothing happens if a is null | unset, else, output $b
          
          a='abcde'; 
          ${a: -1}                  # 'cde' ${param:offset}, note the space if [offset -lt 0]
          ${a:2:2}                  # 'cd' ${param:offset:length}
     ...
     + ~$(...)~ means: execute the command in the parens in a subshell and return its stdout.
       - ~user=$(whoami)~
     + ~(...)~ means: execute the commands listed in the parens in a subshell. 
       - ~a=1;(a=2; echo $a); echo $a~       # 2\n1
     + ~$((...))~ means: perform arithmetic and return the result of the calculation.
       - ~a=$((1+1)); echo $a~               # 2
     + ~((...))~ means: perform arithmetic, possibly changing the values of shell variables, but return no value
       - ~((a=1+1)); echo $a~                # 2
     + ~${...}~ means: return the value of the shell variable named in the braces.
       - ~echo ${a}, ${SHELL}~               # 2, /bin/bash
     + ~{...}~ means: execute the commands in the braces as a group.
         #+begin_src bash
