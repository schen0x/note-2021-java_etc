...
*** the x86 register and their nomenclature
    ...
    + the table of x86 register structure
      ...
      + E[ABCD]X, ESI, EDI: often used as general purpose registers to store temporary data, [[https://wiki.skullsecurity.org/Registers][more info on register (Skull Security, 2012)]] but the primary purpose are as below (quoted OSCP p348):
        ...
        - *ESP* (stack pointer): most recently referenced location on the stack(pointer to the *top/lower address* of the stack)
        - *EBP* (base pointer): points to the *base/bottom/higher address* of the stack when a function is called, provide easy reference for the function to its own stack frame (via offsets). i.e. the (offset 0 pointer)
...
* Book3_Chapter 14 resources
** TODO resources p440, 3 archives, check maillist
   + p450 example
