...
**** steps: The Ticket-Granting Service (TGS) Exchange (RFC 4120::3.3)
  ...
  + TGS Exchange, client side:
    ...
    #+begin_quote RFC4120 3.3.1. Generation of KRB_TGS_REQ Message 
  ...
  + TGS Exchange, server side, additional checks
    ...
    #+begin_quote PWK p646
    (...), the ticket granting service on the KDC receives the TGS_REQ, and if the SPN exists in the domain, the TGT is decrypted using the secret key known only to the KDC.
    #+end_quote
    ...
    - When the TGS Exchange is complete, the client should ha:e both a session key and a service ticket. Then the service authentication begins.

**** steps: The Client/Server (CS) Authentication Exchange (RFC4120::3.2)
    - AP Exchange Overview:
