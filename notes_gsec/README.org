...
*** NTLM Authentication
    ...
    + steps(a client login to an application server):
      ...
      - 2. client computer *send the username to the server*;
      - 3. application server returns a random value called the *nonce* or *challenge*;
      - 4. client *encrypts the nonce using the NTLM hash*, generate the *response* and send it back to the server.
      - 5. server forwards the *response*, the *username*, and the *nonce* to the DC;
