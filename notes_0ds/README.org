...
* TODO list
  ...
  + proxychain
  + use msfvenom, use the different format.
  + (reflective dll injection)
  + list all privilege escalation techniques
  + bad chars (how to infer)
  + msfvenom, windows, linux payload execute methods summary.
  + (msfvenom hex payload reverse), nasm binary format?
  + (migrate, why, how and without meterpreter)
  ...
  + kerberos (from local admin)
  + post-enum scripts
  + privilege escalation methods (general), DCOM etc.
  + *SeImpersonate Internal && AV Bypassing (fs)*
  + mimikatz av-evasion (list_tokens, KRBs related)
