...
* TODO list
  + jobs control
  + interative shell (py and none py?)
  + nc common cmds
  + wireshark filters (capture and display)
  + (tcpdump)
  + (general osint, cp 6)
  + DNS Enumeration (cp 7 revision)
  + (1 or 2 video, nmap common usage)
---------------------------------------- 30th
  + SMB,NFS,SMTP enumeration? (cp7) is necessary?
  + (Nessus? automated web, check if newer PWK contains the info, or exam detail)
  + (Nikto)?
---------------------------------------- 1st
  + (gobuster and Burp, test in box)
  + *XSS, DirTraversal, SQL (cp9 revisit), test in box*
  + *lfi, test in box*
---------------------------------------- 2nd
  + *sqlmap, test in box*
---------------------------------------- 3rd
...
---------------------------------------- 4th, ++
  + exploit search, p455-476, common fixes, one bytes, other techniques
  + *Windows payload AV bypass(p490-511)*? (check exam examples)
  + *post enumeration scripts (linux, windows, try in box)*
---------------------------------------- 5-9th
  + privilege escalation methods (general), DCOM etc. video, common pattern
---------------------------------------- 10th
