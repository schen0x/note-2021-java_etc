...
*** DLL format (Windows)
    ...
    + info:
      ...
      - implicit linking of a lib/dll, the video 20:40/1:02:27: in linking stage, a Hello.lib (an *import library* for dll) file is created automatically, exports a stub function which redirect the call to a function pointer to the real function. Then when `dumpbin /dependencies Hello.exe`, the `Hello.dll` is also in the list.
