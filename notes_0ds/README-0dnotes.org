...
** Windows Buffer Overflow, SyncBreeze (return to a static library)
  ...
  + Because before the `ret`, the $esp should be directly pointing at the payload ((may need no-op slop since clean-up might happen on the `ret` call)see the function prologue).
