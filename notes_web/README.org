...
** extending vs. intersecting(&) interfaces
   + intersecting combines merge types that has a same name, so that:
     ~type anArg: string | number;~ without thworing an error.
* Chapter 50, OLTP (Online Transaction Processing)
** basic
