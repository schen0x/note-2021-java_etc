...
** *lodash* (library) lodash/fp *compose* and *pipe*
   ...
   #+BEGIN_SRC  javascript
       ...
       transform = losash.compose(wrapInDiv, toLowerCase, trim);
       transform = losash.pipe(trim, toLowerCase, wrapInDiv);
   ...
   *Hence:*
   #+begin_src javascript
       ...
       //const wrap = (type, str) => `<${type}>${str}</${type}>`
       const wrap = type => str => `<${type}>${str}</${type}>`
       transform = losash.pipe(trim, toLowerCase, wrap("div"));
       //this gives: 
       //<div>javascript</div>
   #+end_src
...
** immutable
   ...
   *data in redux should not be mutated*
