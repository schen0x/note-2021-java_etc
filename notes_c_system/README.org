...
initialized to zero; ~automatic~ (equivalent to Java local vars inside a function or scope) and ~register~ variables have *undefined* (i.e., garbage) initial values.
     - *automatic variables*:
       #+begin_quote
